...
def test_spanish_sentiment_analyzer():
    ...
    assert positive_result["polarity"] > 0, f"Polaridad positiva esperada > 0, obtuvo {positive_result['polarity']}"
    ...
    assert negative_result["polarity"] < 0, f"Polaridad negativa esperada < 0, obtuvo {negative_result['polarity']}"
    ...
    assert abs(neutral_result["polarity"]) <= 0.2, f"Polaridad neutra esperada <= 0.2, obtuvo {neutral_result['polarity']}"
    ...
    assert negation_result["polarity"] <= 0.2, f"Texto negativo por negación debería tener polaridad <= 0.2, obtuvo {negation_result['polarity']}"
...
def test_calculate_word_frequency_es():
    ...
    print(f"DEBUG: Frecuencia esperada: {target_count}/{total_words} = {target_count/total_words}")
    ...
    assert expected_min <= frequency <= expected_max, \
...
def test_calculate_big_five_scores_with_spanish_data():
    ...
    spanish_data = {
        "posts": [
            {"text": "Hoy fui a una fiesta con mis amigos, fue muy divertido", "reactions": 15, "comments": 3},
            {"text": "Estoy preocupado por el examen de mañana, me siento ansioso", "reactions": 5, "comments": 1},
            {"text": "Leí un libro sobre filosofía muy interesante", "reactions": 8, "comments": 2},
            {"text": "Ayudé a mi amigo con su proyecto, me gusta cooperar", "reactions": 12, "comments": 4},
            {"text": "Organicé mi semana con un plan detallado", "reactions": 7, "comments": 1}
        ],
        "friends_count": 350,
        "groups": ["Programación Python", "Club de Lectura", "Grupo de Música"],
        "basic_info": {"bio": "Desarrollador de software apasionado por la tecnología y el aprendizaje continuo"}
    }
...
def test_save_results_es():
    ...
    test_data = {
        "posts": [
            {"text": "Estoy contento con mi progreso", "reactions": 10}
        ],
        "friends_count": 150,
        "groups": ["Tecnología"],
