...
class SpanishSentimentAnalyzer:
    ...
    POSITIVE_WORDS = {
        'feliz', 'contento', 'alegre', 'emocionado', 'encantado', 'amor', 'maravilloso',
        'fantástico', 'excelente', 'genial', 'increíble', 'perfecto', 'bueno', 'bonito',
        'hermoso', 'divertido', 'agradable', 'positivo', 'optimista', 'satisfecho',
        'encanta', 'gusta', 'apasiona', 'entusiasma', 'admira'
    }
    
    NEGATIVE_WORDS = {
        'triste', 'deprimido', 'enojado', 'enfadado', 'molesto', 'frustrado', 'asustado',
        'preocupado', 'ansioso', 'nervioso', 'estresado', 'malo', 'horrible', 'terrible',
        'pésimo', 'aburrido', 'cansado', 'agotado', 'desanimado', 'desesperado',
        'odio', 'detesto', 'molesta', 'irrita', 'desagrada'
    }
    
    # Intensificadores y negaciones
    INTENSIFIERS = {'muy', 'mucho', 'realmente', 'totalmente', 'absolutamente', 'extremadamente'}
...
class BigFiveAnalyzer:
    def __init__(self):
        ...
        self.neuroticism_words = [
            'ansioso', 'preocupado', 'nervioso', 'triste', 'enfadado', 'estresado',
            'miedo', 'pánico', 'desesperado', 'culpable', 'deprimido', 'angustiado',
            'inseguro', 'temeroso', 'asustado', 'irritado', 'frustrado', 'abatido'
        ]

        self.extraversion_words = [
            'fiesta', 'amigos', 'social', 'divertido', 'energía', 'hablar', 'grupo',
            'celebración', 'reunión', 'alegre', 'extrovertido', 'risa', 'baile',
            'concierto', 'evento', 'compañía', 'socializar', 'festejo', 'júbilo'
        ]

        self.openness_words = [
            'arte', 'música', 'creativo', 'innovador', 'imaginación', 'curioso',
            'aprender', 'filosofía', 'viajar', 'cultura', 'libro', 'película',
            'nuevo', 'diferente', 'experiencia', 'descubrir', 'explorar', 'conocimiento',
            'leer', 'educación', 'tecnología', 'ciencia'
        ]

        self.agreeableness_words = [
            'amable', 'compasivo', 'ayudar', 'cooperar', 'empatía', 'perdonar',
            'generoso', 'considerado', 'paciente', 'apoyar', 'amor', 'cariño',
            'bondad', 'respeto', 'solidaridad', 'comprender', 'escuchar', 'colaborar'
        ]
        ...
        self.conscientiousness_words = [
            ...
            'metas', 'logro', 'estudio', 'proyecto', 'deadline', 'eficiente',
            'productivo', 'ordenado', 'sistemático', 'constante'
        ]
    ...
    def calculate_big_five_scores(self, data: Dict) -> Dict[str, float]:
        ...
        extraversion_score = (
            ...
            min(total_reactions / max(len(posts_text), 1) / 50, 1.0) * 0.4 +  # Reacciones por post
        ...
        agreeableness_score = (
            ...
            min(total_comments / max(len(posts_text), 1) / 10, 1.0) * 0.2  # Normalizar comentarios
        )
        ...
        conscientiousness_score = (
            self.calculate_word_frequency(posts_text, self.conscientiousness_words) * 0.6 +
            (1.0 if len(posts_text) >= 5 else 0.3) * 0.2 +  # Consistencia (mínimo 5 posts)
            (1.0 if bio_text and len(bio_text.strip()) > 20 else 0.3) * 0.2  # Biografía completa
        )
        ...
        self.results = {
            ...
            "calculated_components": {
                "extraversion": {
                    ...
                    "word_frequency": self.calculate_word_frequency(posts_text, self.extraversion_words)
                },
                "neuroticism": {
                    "negative_ratio": sentiment["negative"] / max(sentiment["total_texts_analyzed"], 1),
                    "sentiment_balance": sentiment["sentiment_balance"],
                    "word_frequency": self.calculate_word_frequency(posts_text, self.neuroticism_words)
                }
            }
        }
