...
from config import (
    COOKIES_PATH, WAIT_TIMES, USER_AGENT, SELECTORS,
    FACEBOOK_EMAIL, FACEBOOK_PASSWORD, CREDENTIALS_PROVIDED,
    MAX_POSTS, HEADLESS_BROWSER
)
...
class FacebookScraper:
        ...
        if COOKIES_PATH.exists():
            try:
                ...
                if cookies and self.context:
                    ...
                    self.page.goto("https://facebook.com", wait_until="domcontentloaded")
                    self.random_wait("short")
                    
                    if "facebook.com/home" in self.page.url or self.page.locator(SELECTORS["email_input"]).count() == 0:
    ...
    def automatic_login(self) -> bool:
        ...
        try:
            ...
            if any(x in self.page.url for x in ["facebook.com/home", "facebook.com/?sk=welcome"]):
                logger.info("✅ Login automático exitoso")
                self.save_cookies()
                return True
            
            # Verificar si hay captcha o 2FA
            if self.page.locator("text=/verifica tu identidad|security check/i").count() > 0:
    ...
    def extract_posts_optimized(self, max_posts: int = None) -> List[Dict]:
                ...
                for i in range(min(count, 20)):  # Procesar máximo 20 por iteración
                    ...
                    try:
                        ...
                        if reactions_elem.count() > 0:
                            try:
                                reactions_text = reactions_elem.get_attribute("aria-label", timeout=1000) or ""
        ...
        logger.info("✅ %d posts extraídos en %.1f segundos", len(posts), time.time() - start_time)
        return posts
